...
programmer); automated commits (the daily snapshot bot) are left out. Teammates are listed under `"contributors"` in `package.json`, each with
the name to show and their git author name (`{ "name": "Rishit", "git":
"g4_0884" }`); anyone listed appears even before their first commit, and
